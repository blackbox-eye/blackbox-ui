...
## [v1.6.28] – 2025-12-30 (P1 Scroll Fix)

### Fixed

- **Critical Scroll-Lock Bug Resolution**
  - Root cause: scroll-contract.css loaded via async `preload` was racing with other CSS files
  - Changed scroll-contract.css from async preload to synchronous `<link rel="stylesheet">`
  - Added inline JS failsafe (`#p1-scroll-failsafe`) that injects scroll-unlock styles immediately
  - This ensures `overflow-y: auto !important` is applied before any other CSS can lock scroll

### Technical Details

- **CSS Load Order Fix**: scroll-contract.css now loads synchronously AFTER all preloaded CSS
- **JavaScript Failsafe**: Inline script injects emergency scroll-unlock styles in `<head>`
- **Affected Files**: `includes/site-header.php`
- **Test Coverage**: Home page, demo.php, about.php verified for scroll functionality
