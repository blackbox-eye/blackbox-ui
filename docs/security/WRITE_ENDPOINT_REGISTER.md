...
| `api/vault-download.php` | `GET` | Authenticated vault download/decrypt | Session auth required visible | N/A for current GET-only behavior | Not applicable | Very high document sensitivity | REPO-VERIFIED | Low as write surface | Keep outside write-endpoint PR |
...
- GET-only endpoints are listed only to show the boundary between write surfaces and adjacent read-only APIs.
