...
function Save-Target($name, $path) {
  ...
  # Ignore <noscript> fallbacks when checking primary load strategy.
  $headSansNoScript = [regex]::Replace($head, '(?is)<noscript.*?</noscript>', '')
  $checks = @(
    @{ label='BBX_HEAD_MARKER'; pattern='BBX_HEAD_MARKER' },
    @{ label='css_version meta'; pattern='name="css_version"' },
    @{ label='scroll-contract rel=stylesheet (primary, not noscript)'; pattern='<link[^>]+rel="stylesheet"[^>]+scroll-contract\.css' },
  ...
  foreach ($c in $checks) {
    $candidate = $(if ($c.label -like '*primary, not noscript*') { $headSansNoScript } else { $head })
    $found = [regex]::IsMatch($candidate, $c.pattern, [System.Text.RegularExpressions.RegexOptions]::IgnoreCase)
