...
async function getCenterHitMatch(locator) {
  ...
  test.describe(`Agent Access - ${deviceName} (${viewport.width}x${viewport.height})`, () => {
    ...
    if (deviceName !== "desktop") {
      test("Quick Switch should not overlay hero section", async ({ page }) => {
        ...
        const quickSwitchBox = await page
          .locator(".console-selector__quick")
          .boundingBox();
        ...
        if (heroBox && quickSwitchBox) {
          ...
          expect(quickSwitchBox.y).toBeGreaterThanOrEqual(
            heroBox.y + heroBox.height - 5,
          );
        }
      });

      test("Quick Switch should be in normal flow (not sticky)", async ({
        page,
      }) => {
        const quickSwitch = page.locator(".console-selector__quick");

        // Get computed style
        const position = await quickSwitch.evaluate(
      ...
      test("Console cards should not be covered by sticky CTA", async ({
        page,
      }) => {
        ...
        const isStickyCTAVisible = await stickyCTA
          .isVisible()
          .catch(() => false);
      ...
      test("Console cards should be clickable and not obscured", async ({
        page,
      }) => {
        const consoles = ["ccs", "gdi", "intel24"];

        for (const consoleId of consoles) {
          const card = page.locator(
            `.console-card[data-console="${consoleId}"]`,
          );
          const cta = card.locator(".console-card__cta");
          ...
          if (isDisabled) {
            ...
            const requestButton = card.locator(
              '[data-testid="intel24-request-btn"]',
            );
            await requestButton.scrollIntoViewIfNeeded();
            await expect(requestButton).toBeVisible();

            const requestHit = await getCenterHitMatch(requestButton);
            expect(
              requestHit.matches,
              `Request-access button should be clickable for ${consoleId}`,
            ).toBeTruthy();
            await requestButton.click({ trial: true });
            continue;
          }

          const hit = await getCenterHitMatch(cta);
          expect(
            hit.matches,
            `CTA should be clickable for ${consoleId}`,
          ).toBeTruthy();
      ...
      test("should handle safe area insets properly", async ({ page }) => {
        ...
        const paddingLeft = await mainContent.evaluate(
          (el) => window.getComputedStyle(el).paddingLeft,
        );
        const paddingRight = await mainContent.evaluate(
    ...
    test("Console cards should be visible and properly styled", async ({
      page,
    }) => {
    ...
    test("should not have z-index stacking issues", async ({ page }) => {
      ...
      if ((await hero.isVisible()) && (await selector.isVisible())) {
        const heroZIndex = await hero.evaluate(
          (el) => window.getComputedStyle(el).zIndex,
        );
        const selectorZIndex = await selector.evaluate(
          (el) => window.getComputedStyle(el).zIndex,
        );

        // Hero should have higher z-index than selector
        expect(parseInt(heroZIndex) || 0).toBeGreaterThanOrEqual(
          parseInt(selectorZIndex) || 0,
        );
      }
    });
    ...
    test("should take screenshot for visual verification", async ({
      ...
      await testInfo.attach(
        `screenshot-${browserName}-${deviceName}-${viewport.width}x${viewport.height}`,
        {
          body: screenshot,
