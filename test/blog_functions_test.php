...
assert_equals(0, bbx_get_blog_posts_json_count(), 'Missing posts key should be treated as empty array');
assert_equals([], bbx_get_blog_posts_from_json(), 'Missing posts key should return empty posts');

// Test Case: Root JSON as plain array
write_mock_json([1, 2, 3]);
assert_equals(0, bbx_get_blog_posts_json_count(), 'Root JSON as plain array should return fallback structure');
assert_equals([], bbx_get_blog_posts_from_json(), 'Root JSON as plain array should return empty posts');
...
// Test Case: Post with tags present but not an array
$invalid_tags_post = $valid_json;
$invalid_tags_post['posts'][] = [
    'id' => '5',
    'title' => 'Post 5',
    'region' => 'DK',
    'tags' => 'NotAnArray', // Tags is a string
    'published_at' => '2025-11-13T10:00:00Z'
];
write_mock_json($invalid_tags_post);
assert_equals(0, bbx_get_blog_posts_json_count(null, 'NotAnArray'), 'Post with string tags should not crash and should not match tags');
...
write_mock_json('content');
chmod($tmpRoot . '/blog/posts.json', 0000);
$data = bbx_load_blog_posts_json();
assert_equals('1.0.0', $data['version'], 'Unreadable file should return fallback structure');
assert_equals([], $data['posts'], 'Unreadable file should have empty posts');
chmod($tmpRoot . '/blog/posts.json', 0644); // Restore permissions for subsequent tests
